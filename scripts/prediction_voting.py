...
def write_csv(id, pred_ud, pred_close, file_name):
	...
	pred_close = pred_close.astype(dtype='float32').flatten()
	data = {'ETFid':pred_id, \
...
if __name__ == "__main__":
	...
	model_close.fit( x_train_close, y_train_close, batch_size=100, epochs=80, validation_split=0.1, shuffle=True, verbose=1, callbacks=[earlyStopping])
	...
	for i in range(voting_size_dnn):
		# sequence length(觀察的天數~= time_frame - day_offset) ; feature length (7 dims)
		...
		model_ud.fit( x_train_ud[::-1], y_train_ud[::-1], batch_size=100, epochs=80,validation_split=0.1, shuffle=True, verbose=1, callbacks=[earlyStopping])

		# Use trained model to predict
		pred_ud_tmp = model_ud.predict(x_test_ud)
		print('pred_ud_tmp.shape'+str(pred_ud_tmp.shape))
		pred_ud_tmp = np.sign(pred_ud_tmp)

		if i == 0:
			pred_ud_all = pred_ud_tmp
		else:
			pred_ud_all = np.concatenate((pred_ud_all, pred_ud_tmp), axis= 0) 



	for i in range(voting_size_rf):
		forest = ensemble.RandomForestClassifier(n_estimators=6, random_state=42,max_features=11, min_samples_leaf=59)
		multi_target_forest = MultiOutputClassifier(forest, n_jobs=-1)
		multi_target_forest.fit(x_train_ud, y_train_ud)
		pred_ud_tmp = multi_target_forest.predict(x_test_ud)
		pred_ud_tmp = np.sign(pred_ud_tmp)
		pred_ud_all = np.concatenate((pred_ud_all, pred_ud_tmp), axis= 0) 

	pred_ud = pred_ud_tmp
	for i in range(pred_ud.shape[0]):
		for j in range(pred_ud.shape[1]):
			counts = np.bincount(pred_ud_all[i::voting_size,j].astype(int)+1)
			pred_ud[i][j] = np.argmax(counts) - 1

	print('pred_ud.shape->'+str(pred_ud.shape))
	#------------------------#
	#		Evaluation		 #
	#------------------------#
